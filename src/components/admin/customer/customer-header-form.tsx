...
interface CustomerHeaderFormProps {
    name: string
    email: string
    status: string
    isSaving: boolean
...
const CustomerHeaderForm = ({ name, email, status, isSaving, onSave, backHref = "/admin/customers" }: CustomerHeaderFormProps) => {
  return (
            ...
            <div className="mx-auto flex max-w-8xl items-center justify-between px-4 py-2.5 sm:px-6">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-bold text-foreground sm:text-lg">{name || "New Customer"}</h1>
                            <p className="text-xs text-muted-foreground">{email || "newcutomer@email.com"}</p>
                        </div>
                        {status === "ACTIVE" && (
                        ...
                        {status === "SUSPENDED" && (
                            <Badge variant="outline" className="ml-2 hidden capitalize sm:inline-flex bg-red/10 text-red-500 border-primary/20">
                                SUSPEND
                            </Badge>
                        )}
                    </div>
